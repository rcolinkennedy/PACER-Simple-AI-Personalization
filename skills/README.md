...
| `brand-voice/` | Documenting, building, or reviewing content against a brand voice. Load when creating a new brand voice config, reviewing content for brand consistency, adapting tone for different audiences, or enforcing style/terminology. Load alongside the relevant `context/brand-voices/<name>.md` config. |
| `task-management/` | Managing cross-session tasks via `TASKS.md`. Load when user asks about tasks, wants to add/complete items, or at session wrap for Notion sync. Handles the PACER ↔ Notion Active Context sync protocol for Lite embodiment visibility. |
| `skill-imports/` | Importing an external skill into PACER. Load when "let's import skill X" or when evaluating a skill for portability. Covers pre-import discussion, mode selection, access audit, file creation, and post-import housekeeping. |
